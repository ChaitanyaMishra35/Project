...
login_manager.login_view='self_profile'
...
def feed():
    ...
    return render_template('feed.html',count_likes=count_likes,nof=nof,nofi=nofi,users2=users2,current_user=current_user.username,months=months,dp=iages,all_users=all_users,users_fname=zip(users_fname,users_lname), pp=pp,
...
def like_comment_share(args):
    ...
    elif var=='like' and current_user.username not in all_likers:
...
def count_follow(username):
    ...
    for i in uu:
        if i.username==username and i.followers!='':
            a1+=1
        elif i.username==username and i.following!='':
            a2+=1
